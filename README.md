...
### Fall 2026 course notebooks

For the Fall 2026 semester, the following six course notebooks have been updated and verified to run on Google Colab with Python 3.13: Chapter 1, Introduction; Chapter 2, Text Classification; Chapter 3, Transformer Anatomy; Chapter 4, Multilingual NER; Chapter 5, Text Generation; and Chapter 10, Training Transformers from Scratch.

Please note the following when running the exercises:

- Start with a fresh Colab GPU runtime and run each notebook in order from its first installation cell.
- The TensorFlow exercises in Chapter 2 are not included in the default course scope.
- Chapter 10 uses a Colab-friendly demo configuration focused on tokenizer training. The original large-scale data processing and long pretraining run are not part of the default execution path.
- The default exercises do not require signing in to the Hugging Face Hub or Weights & Biases. Model uploads and external experiment tracking remain optional and require separate authentication.
- Notebooks outside these six chapters, and environments other than Google Colab, were not included in this modernization and verification effort.
